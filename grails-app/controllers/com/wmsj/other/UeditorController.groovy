...
class UeditorController {
    ...
    Map<String,List<String>> fileMap=['image':["gif","png","jpg","jpeg","bmp"],
    'file':["rar","zip","doc","docx","pdf","txt","xls","xlsx"],
        ...
        /* 前后端通信相关的配置,注释只允许使用多行方式 */
{
    ...
    "imageMaxSize": 40960000, /* 上传大小限制，单位B */
...
//        def filepart="/${xtype}/${new Date().format("yyyyMM")}/${filename}";
        def filepath="";
        String message = "文件类型不允许";
        if(fileMap[xtype].contains(extName)){
            filepath=baseUrl+filepart;
//            filepath=servletContext.getRealPath(baseUrl)+filepart;
        ...
        if( callback == null ){
            ...
            render ("<script>"+ callback +"(${result as JSON})</script>");
        }
    }
    ...
    def list(String xtype) {
        def baseUrl=grailsApplication.config.ueditor.upload.baseUrl;
        def userId = springSecurityService.currentUserId
        def uri = File.separator+"${userId}"+File.separator+"${xtype?.capitalize()}"+File.separator;
        File file = new File(baseUrl+uri);
        File[] tempList = file.listFiles();
        def length = tempList?.length;
        def map = ["state": "SUCCESS","start": params.start,"total": length];
        def list = []
        def count = params.start?.toInteger()+params.size?.toInteger();
        def len = count
        if(count > length){
            len = length
        }
        for (int i = params.start?.toInteger(); i < len; i++) {
            def tm = [:];
            tm.url = "${request.contextPath}/ueditor/download?path=${uri+tempList[i].name}";
            list << tm;
        }
        map.list = list;
        render map as JSON;
